...
```{r}
topfeatures(textoMatriz)
```

Estas palabras no nos dicen nada acerca de las conversaciones que tienen los empleados de enron, así que comencemos a filtrar palabras y expresiones comunes que tengan poco significado para nuestro análisis.
...
palabras2 <- tokens(enronCorpus, 
                            ...
                            remove_url = TRUE,
                            remove_symbols = TRUE)
my_stopwords <- c(as.character(c(0:9)), stopwords("english"), "now")

enronMatriz2 <- dfm(palabras2, tolower = TRUE)
enronMatriz2 <- dfm_remove(enronMatriz2, my_stopwords)
print("'Top features' de la matriz sin las stopwords: ")
