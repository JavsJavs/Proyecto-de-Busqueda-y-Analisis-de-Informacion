...
```{r}
mailSample <- sample(edges.full$body, 1000)
length(mailSample)
```
...
```{r}
mailSample[6]
sentimentExample <- analyzeSentiment(mailSample[6])
str(sentimentExample)
...
```{r}
str(sentimentOverall)
sentimentOverallBinary <- convertToBinaryResponse(sentimentOverall)$SentimentQDAP
```

```{r}
summary(sentimentOverall$SentimentQDAP)
hist(sentimentOverall$SentimentQDAP)
```

```{r}
sentimentOverall2 <- sentimentOverall
summary(sentimentOverall$SentimentHE)
hist(sentiment$SentimentHE)
```
